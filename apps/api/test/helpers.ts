...
export async function buildTestApp(dir: string): Promise<{ app: FastifyInstanceLike; cleanup: () => void }> {
  const db = openDb(join(dir, "test.sqlite"));
  migrate(db);
  seedDevData(db);
  const app = (await buildApp({ db, storeDir: join(dir, "artifacts"), token: "dev-token" })) as FastifyInstanceLike;
