...
| Historical onboarding | VERIFIED* | Reconstruction executed end to end (see History row): LIVE regeneration published 144 verified captures (13 commits × 12 scenario-recipes) + 12 expected-failure records, 0 gaps; regression test drives the normal plan path (plan → run → worker → published captures for the selected commit sha). *Scope bound to the static fixture corpus — the React/Vite build adapter for real-app historical revisions is in flight (vorflux/closure-2) |
...
| # | Finding | Status |
| --- | --- | --- |
| B1a | Worker image grounding reads bytes from the local fs only; S3 fallback URL is auth-gated → provider fetch 401 | fix in flight (vorflux/closure-2: worker reads through the configured storage driver, inline bytes to the provider; artifact authorization preserved) |
| B1b | Ignored-image (degraded) notes dropped by both persistence paths; never reach the proposal DTO or editor | fix in flight (vorflux/closure-2: persist degraded note, expose in DTO, render in editor) |
| B2a | History execution ignores the plan's selected scenarioIds (always runs all 12) | fix in flight (vorflux/closure-2: plan-validated scenario set drives execution, estimates, completion) |
| B2b | Reconstruction serves source trees statically; no React/Vite build adapter | fix in flight (vorflux/closure-2: constrained build adapter + buildable React/Vite fixture corpus) |
